...
{
    class HEICConverterExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/heicconverter";

        /// <summary>
        /// Make a GET request to the HEIC Converter API
        /// </summary>
        static async Task<JsonDocument> CallHEICConverterAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling HEIC Converter API...\n");

            var result = await CallHEICConverterAPI();
