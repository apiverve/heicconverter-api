...
class HeicconverterValidationException implements Exception {
  final List<String> errors;

  HeicconverterValidationException(this.errors);

  @override
  String toString() => 'HeicconverterValidationException: ${errors.join("; ")}';
}
...
/// HEIC Converter API client.
///
/// For more information, visit: https://apiverve.com/marketplace/heicconverter?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [image] (required) - Upload a HEIC or HEIF image file to convert
...
class HeicconverterClient {
  ...
  HeicconverterClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/heicconverter',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HeicconverterValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HeicconverterValidationException(errors);
    }
  }

  /// Execute a request to the HEIC Converter API.
  ///
  /// Parameters are validated before sending the request.
  Future<HeicconverterResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HeicconverterException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HeicconverterResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HeicconverterException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HeicconverterException('Resource not found');
      } else {
        throw HeicconverterException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HeicconverterException) rethrow;
      throw HeicconverterException('Request failed: $e');
    }
  }

  /// Execute a request to the HEIC Converter API with a file upload.
  ///
  /// [filePath] - Path to the file to upload
  /// [additionalParams] - Optional additional parameters to send with the request
  Future<HeicconverterResponse> executeWithFile(
    String filePath, {
    ...
    try {
      ...
      if (!await file.exists()) {
        throw HeicconverterException('File not found: $filePath');
      }
      ...
      if (e is HeicconverterException) rethrow;
      throw HeicconverterException('File upload failed: $e');
    }
  }
  ...
  Future<HeicconverterResponse> executeWithBytes(
    List<int> bytes,
    String fileName, {
    Map<String, String>? additionalParams,
  }) async {
...
/// Exception thrown by the HEIC Converter API client.
class HeicconverterException implements Exception {
  final String message;

  HeicconverterException(this.message);

  @override
  String toString() => 'HeicconverterException: $message';
}
