...
class HeicconverterAPIClientError(Exception):
...
class ValidationError(HeicconverterAPIClientError):
...
class HeicconverterAPIClient:
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the HeicconverterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises HeicconverterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/heicconverter'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise HeicconverterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[HeicconverterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns HEICConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises HeicconverterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise HeicconverterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise HeicconverterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise HeicconverterAPIClientError(f"Request failed: {str(e)}")
    ...
    def execute_with_file(self, files: Dict[str, Any], params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log(f"Request exception: {str(e)}")
            raise HeicconverterAPIClientError(f"File upload failed: {str(e)}")
    ...
    def execute_with_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not url or not isinstance(url, str):
            raise HeicconverterAPIClientError("URL must be provided as a string.")

        if not re.match(r'^https?://.+', url, re.IGNORECASE):
            raise HeicconverterAPIClientError("Invalid URL format. URL must start with http:// or https://")
